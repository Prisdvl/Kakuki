...
export default function CategoryManage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await request.get("/categories/manage/");
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editing) {
        await request.put(`/categories/manage/${editing.id}/`, values);
        message.success("更新成功");
      } else {
        await request.post("/categories/manage/", values);
  ...
  const handleDelete = async (id) => {
    try { await request.delete(`/categories/manage/${id}/`); message.success("删除成功"); fetchData(); }
  ...
  return (
    <div>
      ...
      <Modal title={editing ? "编辑分类" : "新增分类"} open={modalOpen} onOk={handleSubmit} onCancel={() => { setModalOpen(false); setEditing(null); }}>
