/**
 * 生成 seed/seed.sql —— 幂等演示数据（对照 Django seed_demo）
 * 用法：node seed/build-seed.mjs && wrangler d1 execute kakuki-db --local --file seed/seed.sql
 * 密码哈希格式与 src/util.ts 的 verifyPassword 一致：pbkdf2_sha256$100000$<salt>$<b64hash>
 */
...
const now = Date.now();
const daysAgo = (n) => new Date(now - n * 86_400_000).toISOString().replace(/\.\d{3}Z$/, 'Z');

/* ---------- 演示内容（复刻 backend seed_demo.py） ---------- */

const DEMO_ADMIN = { username: 'Prisdvl', password: '***REDACTED***', nickname: 'Prisdvl' };
const DEMO_USER = { username: 'demo', password: 'demo123456', nickname: 'Demo' };
...
const projects = [
  ['Kakuki Blog', '本项目：Django + React 全栈个人博客，玻璃拟态设计、在线音乐播放器、LeetCode 数据可视化。', 'React,Django,DRF,MySQL', 1, 0],
  ['AI Chatbot', '基于大模型 API 的智能对话机器人，支持多轮上下文与流式输出。', 'Python,FastAPI,WebSocket', 0, 1],
  ['LeetCode Tracker', '自动同步刷题记录的数据看板，支持难度分布与连续打卡统计。', 'React,ECharts,GitHub Actions', 0, 2],
  ['Music Player', '独立版在线音乐播放器，支持自定义歌单与歌词高亮。', 'Vue,Express,SQLite', 0, 3],
];

/* ---------- 生成 SQL ---------- */

const lines = [];
lines.push('-- Kakuki 演示数据（由 seed/build-seed.mjs 生成，幂等可重复执行）');
lines.push('-- 演示账号：Prisdvl / ***REDACTED***（管理员）· demo / demo123456（普通用户）');
lines.push('');
lines.push('DELETE FROM article_likes; DELETE FROM talk_likes; DELETE FROM sqlite_sequence WHERE name IN (' +
  "'articles','comments','talks','projects','categories','article_likes','talk_likes');");
lines.push('DELETE FROM comments; DELETE FROM talks; DELETE FROM projects; DELETE FROM articles; DELETE FROM categories;');
lines.push('');

// 用户（INSERT OR IGNORE：保留已改密的账号）
for (const [u, isStaff] of [[DEMO_ADMIN, 1], [DEMO_USER, 0]]) {
  lines.push(
    `INSERT OR IGNORE INTO users (username, password, nickname, avatar, is_staff, is_superuser, date_joined) VALUES (${q(u.username)}, ${q(hashPassword(u.password))}, ${q(u.nickname)}, '/github-avatar.jpg', ${isStaff}, ${isStaff}, ${q(daysAgo(120))});`,
  );
}
lines.push('');
...
categories.forEach(([name, desc], i) => {
  lines.push(`INSERT INTO categories (id, name, description) VALUES (${i + 1}, ${q(name)}, ${q(desc)});`);
});
lines.push('');
...
articles.forEach((a, i) => {
  ...
  lines.push(
    `INSERT INTO articles (id, title, content, summary, category_id, author_id, views, is_top, created_at, updated_at) VALUES (${i + 1}, ${q(a.title)}, ${q(a.content)}, ${q(a.summary)}, ${a.category + 1}, 1, ${a.views}, ${a.isTop ? 1 : 0}, ${q(createdAt)}, ${q(createdAt)});`,
  );
});
lines.push('');

// 评论（作者 = Prisdvl，对齐 seed_demo）
comments.forEach(([idx, content], i) => {
  lines.push(
    `INSERT INTO comments (id, article_id, user_id, content, parent_id, created_at) VALUES (${i + 1}, ${idx + 1}, 1, ${q(content)}, NULL, ${q(daysAgo(2 - (i % 2)))});`,
  );
});
lines.push('');

// 杂谈
talks.forEach((content, i) => {
  lines.push(`INSERT INTO talks (id, content, author_id, views, created_at) VALUES (${i + 1}, ${q(content)}, 1, ${20 + i * 7}, ${q(daysAgo(i))});`);
});
lines.push('');

// 项目
projects.forEach(([name, desc, tech, featured, order], i) => {
  lines.push(
    `INSERT INTO projects (id, name, description, url, repo_url, tech_stack, is_featured, sort_order, created_at) VALUES (${i + 1}, ${q(name)}, ${q(desc)}, '', '', ${q(tech)}, ${featured}, ${order}, ${q(daysAgo(30 - i * 5))});`,
  );
});
lines.push('');
...
// 演示点赞（游客 IP 维度）
lines.push(
  `INSERT INTO article_likes (article_id, user_id, ip_address, created_at) VALUES (1, NULL, '203.0.113.7', ${q(daysAgo(1))}), (2, NULL, '203.0.113.9', ${q(daysAgo(1))}), (3, NULL, '198.51.100.4', ${q(daysAgo(2))});`,
);
lines.push(
  `INSERT INTO talk_likes (talk_id, user_id, ip_address, created_at) VALUES (1, NULL, '203.0.113.7', ${q(daysAgo(1))}), (2, NULL, '198.51.100.8', ${q(daysAgo(2))});`,
);
lines.push('');

writeFileSync(join(__dirname, 'seed.sql'), lines.join('\n'), 'utf8');
console.log('seed/seed.sql 生成完毕');
